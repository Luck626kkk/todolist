            ...
            $(document).ready(function () {
                ...
                $("#list").on("blur",".content",function(e){
                    ...
                    if(isNew){
                        ...
                        $.post('http://localhost:8080/todo/update.php',{ id: id, content: content});
                ...
                $("#list").on("click",".delete",function(e){
                    ...
                    if(result){
                        // AJAX call
                        var id = $(this).closest('li').data('id');
                        $.post('http://localhost:8080/todo/delete.php',{ id: id},function(data,textStatus,xhr){
                ...
                $("#list").on("click",".checkbox",function(e){
                    //AJAX call
                    var id = $(this).closest('li').data('id');
                    $.post('http://localhost:8080/todo/complete.php',{ id: id},function(data,textStatus,xhr){
                ...
                $("#list").find("ul").sortable({
                    ...
                    stop: function(){
                        var orderPair =[];
                        $("#list").find("li:not(.new)").each(function(index, li){
                            //var id ;
                            //var order ; 
                            orderPair.push({
                                id: $(li).data("id"),//需要先用jquery 包起來li
                                order: index +1 
                            });
                        });
                        $.post('http://localhost:8080/todo/sort.php',{ orderPair: orderPair});
                            // 裡面不需要再做事情了
                    },
                });
            });
        ...
        <div id="panel" style="padding-bottom: 50px;">
            ...
            <div id="list">
                <ul>
                    ...
                    <li class="clearfix  <?php if($todo["is_complete"] == 1){echo"complete";}?>" data-id="<?=$todo['id']?>" >
